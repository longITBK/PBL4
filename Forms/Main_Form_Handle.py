...
class Main_Form_Handle(Ui_Form, Style, Scripts, Support):
    ...
    def Battery_Details_Updating(self):       
        ...
        self.Percentage_bar.setValue(int(percentage.stdout.decode('utf-8').strip().replace('%','')))
        ...
        s = int(percentage.stdout.decode('utf-8').strip().replace('%',''))
        if s==100:
            self.Hide_Charging_Sign()
    ...
    def Night_Light_Updating(self):
        ...
        else: self.cbb_schedule.setCurrentText('Manual Schedule')        
